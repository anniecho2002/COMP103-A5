...
public class OrganisationChart {
    ...
    private Position findPosition(double x, double y, Position pos){
        ...
        if(pos.on(x,y)){ return pos; }
            ...
            if(pos.isManager()){
                for(Position p: pos.getTeam()){
                  Position ans = findPosition(x, y, p); // try again with all the different team members
    ...
    private void movePosition(Position pos, Position target) {
        ...
        if(pos == organisation){ return; }                // cannot move the ceo
        if(inSubtree(pos, target) == false){
    ...
    public void removePosition(Position pos){
        if(pos == null){ return; }
            ...
            Position manager = pos.getManager();
            if(manager != null && pos.getTeam().size() == 0){ manager.removeFromTeam(pos); }
        }
    }

    /** [COMPLETION:]
     * Cannot move a position down into its own subtree
    ...
    public void saveTree(){
        String filename = UIFileChooser.save("Filename: ");
        PrintStream output = new PrintStream(filename);
        saveNode(output, organisation);
        output.close();
    }
    
    /**
     * Recurvsive method that prints to the output
     * For saving the tree into a file
     */
    public void saveNode(PrintStream output, Position pos){
        if(pos != null){
            String role = pos.getRole();
            int teamNum = pos.getTeamNum();
            output.println(role + " " + teamNum);
            for(Position p: pos.getTeam()){
                saveNode(output, p);
            }
        }
    ...
    public void loadTree(){
        try{
            String filename = UIFileChooser.open("Open a file: ");
            allLines.clear();
    ...
    public void loadNode(Position manager, double offset){
        if(index < allLines.size()){
            Scanner sc = new Scanner(allLines.get(index));
            String role = sc.next();
            int teamNum = sc.nextInt();
            Position p = new Position(role);
            p.setOffset(offset);
            if(manager == null){ organisation = p; }
            else { p.addToManagersTeam(manager); }
            offset = (teamNum * 20 * -1) - 20;
            for(int i=0; i<teamNum; i++){
                index++;
                loadNode(p, offset + (i * 60));
            }
        }
    }
